...
#' Retrieve the coefficients of associations between the explanatory and response variables for a MBPLSDA object
#'
#' @param res the output of `packMBPLSDA::mbplsda` (the input of `packMBPLSDA::boot_mbplsda`)
#' @param boot (optional) the output of `packMBPLSDA::boot_mbplsda` to display the confidence intervals
...
get_mtbda_coef <- function(res, boot = NULL, Y_var = NULL, CI = 0.95, as_matrix = FALSE) {
  ...
  if (!is.null(boot)) {
    boot_res <-
      purrr::map(
        ...
        .f = function(Yvar, boot) {
          boot$XYcoef[[Yvar]] %>%
            dplyr::select(variables, Q2.5, Q97.5) %>%
            dplyr::rename(variable = variables, lo = Q2.5, up = Q97.5) %>%
            dplyr::mutate(Yvar = Yvar)
      },
    boot = boot
      ) %>%
      purrr::list_rbind() %>%
    ...
    coefs <- coefs %>% left_join(boot_res, by = join_by(variable, Yvar))
